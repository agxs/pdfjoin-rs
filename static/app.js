...
function getElements() {
    const list = must("file-list");
    const input = must("file-input");
    const zone = must("drop-zone");
    const submit = must("submit-button");
    const clear = must("clear-button");
    const progress = must("progress-bar");
    const status = must("status-text");
    const error = must("error-text");
...
function submitJoinRequest(files) {
